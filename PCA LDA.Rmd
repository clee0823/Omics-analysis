...
setwd("C:/Users/clfee/Downloads/detect and correct mislabeled samples using rich multi-omics datasets")
rm(list=ls())
...
msi_toplist <- fread('msi_toplist.csv')[-1,-1] # results generated from Limma analysis
...
Since PCA is variance maximize exercise. Therefore, we need to scale the data to comapre the varaiance at the same level.
```
minmx <- function(x){return((x- min(x)) /(max(x)-min(x)))  }

```
# Replace NA with zero. 
Low abundant protein will generate NAs in spectral count based data.
```
...
df.pc <- prcomp(dat, center = TRUE, scale = FALSE) # center data to zero
